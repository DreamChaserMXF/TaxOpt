...
@dataclass
class TaxConfig:
    basic_deduction: int = 60000  # 基本减除费用（年，通常 60000）
    ...
    housing_fund_rate: float = 0.12  # 公积金个人比例
...
def validate_tax_config(c: TaxConfig) -> None:
    ...
    for name, r in (
        ...
        ("housing_fund_rate", c.housing_fund_rate),
    ):
        if not 0 <= r <= 1:
...
class TaxCalculator:
    ...
    def monthly_social_security(self, monthly_salary: float) -> Dict[str, float]:
        ...
        fund = housing * self.c.housing_fund_rate
    ...
    def comprehensive_annual_tax_and_social_security(self, monthly_salary: float) -> Tuple[float, float]:
        annual_salary = monthly_salary * 12
        additional = self.annual_additional_total()
        social_security = self.annual_social_security_total(monthly_salary)
        taxable = annual_salary - self.c.basic_deduction - additional - social_security
        tax = _progressive_tax(taxable, COMPREHENSIVE_BRACKETS)
    ...
    def total_tax_and_social_security(self, monthly_salary: float, bonus: float) -> Tuple[float, float]:
        annual_tax, annual_social_security = self.comprehensive_annual_tax_and_social_security(monthly_salary)
        bonus_tax = self.bonus_tax(bonus)
        return annual_tax + bonus_tax, annual_social_security

    def monthly_details(self, monthly_salary: float) -> List[Dict[str, Any]]:
        ...
        for month in range(1, 13):
            ...
            cum_spec = spec_m * month
            cum_add = add_m * month + self.c.annual_additional_deduction
            cum_taxable = cum_income - cum_basic - cum_spec - cum_add
            ...
            rows.append(
                {
                    ...
                    "social_security": ss,
                    "tax": month_tax,
                    "after_tax_salary": monthly_salary - spec_m - month_tax,
...
class TaxOptimizer:
    ...
    def optimize(self, total_annual: float, step: int = 1) -> Dict[str, Any]:
        ...
        for m in range(0, cap + 1, step):
            ...
            if bonus < 0:
                continue
            tax, annual_ss = self.calc.total_tax_and_social_security(m, bonus)
            net = total_annual - tax - annual_ss
            if net > best_net or (net == best_net and tax < best_tax_at_net):
                best_net = net
                best_tax_at_net = tax
                best = {
                    "monthly_salary": m,
...
def result_from_salary_bonus_split(
    ...
    tax, annual_ss = calc.total_tax_and_social_security(monthly_salary, bonus)
    bt = calc.bonus_tax(bonus)
    net = total_annual - tax - annual_ss
    return {
        ...
        "effective_burden_rate": (tax + annual_ss) / total_annual if total_annual else 0.0,
        "bonus_tax": bt,
        "bonus_after_tax": bonus - bt,
        "monthly_details": calc.monthly_details(monthly_salary),
    }
...
def print_result(
    ...
    *,
    ...
    print("=" * 80)
    print(title or "个人所得税税筹规划结果（目标：全年到手最多）")
    print("=" * 80)
    ...
    print(f"全年到手收入: {result.get('net_take_home', 0):,.2f} 元")
    ...
    print(f"五险一金占名义收入: {result.get('effective_social_security_rate', 0) * 100:.2f}%")
    print(f"个税+五险一金占名义收入: {result.get('effective_burden_rate', 0) * 100:.2f}%")

    print("\n" + "-" * 80)
    print("月度明细")
    print("-" * 80)
    print(f"{'月份':<6} {'月薪':<12} {'三险一金':<12} {'税额':<12} {'税后工资':<12}")
    print("-" * 80)
    for d in result.get("monthly_details", []):
        print(
            f"{d['month']:<6} {d['salary']:>10,.2f} "
            f"{d['social_security']['total']:>10,.2f} {d['tax']:>10,.2f} "
            f"{d['after_tax_salary']:>10,.2f}"
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="个人所得税：税筹优化或按给定拆分计算全年到手（扣个税与个人五险一金）。"
    )
    parser.add_argument(
        "mode",
        nargs="?",
    ...
    try:
        if args.mode == "optimize":
            ...
            result = TaxOptimizer(calc).optimize(total_income, step)
            if result:
                print_result(result, total_income)
            else:
                print("未找到优化方案")
